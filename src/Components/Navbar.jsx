...
const Navbar = () => {
  ...
  return (
    ...
    <motion.div data-scroll data-scroll-section  data-scroll-speed="0.2" 
    initial={{ y: 0 }}
      animate={{ y: visible ? 0 : -100 }}
      transition={{ duration: 0.3 }}
    className='fixed z-index-[999]  w-full px-10 py-8 font["Neue_Montreal"] flex justify-between items-center'>
        ...
        <div className='links flex text-xl font-[Neue-Montreal]  gap-10'>
          {["Services","Our Work"," About us", " insights","contact"].map((item, index) =>(
            <a key={index} className={`text-lg rounded hover:bg-white hover:text-gray-700 font-[Neue-Montreal]  font-light capitalize ${index===4 && " ml-32"} `}>{item}</a>
            ))}
